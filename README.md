<img src="bg.png" width=100%/>

# 👋 Hello World

My name is Artur Flis and I currently live in Poznań, Greater Poland.

I’ve been working as a frontend developer since 2020. My main focus is UI/UX - I care a lot about clean layout, good visual balance, and making interfaces feel polished. Sometimes I also like to go off the rails and build purely aesthetic websites just for fun.

On the frontend I mainly work with CSS and JavaScript, and I’m currently planning to learn Vue to expand my stack. Outside of frontend work, my favorite programming language is Go because it’s fast, easy to understand, and compiles into a single binary, which makes it great for building practical tools and backend services.

I’m currently studying IT in a technikum and I regularly take part in different IT olympiads and competitions.

In my free time I’m into self-hosting and home infrastructure - I run my own server at home. I also enjoy flying FPV drones (still learning), 3D printing, and I’m in the process of creating my own podcast.

## 🌍 Projects I'm working on
- Dynacat https://github.com/Panonim/dynacat
- Degoog https://github.com/degoog-org/degoog
- Treegraph https://github.com/Panonim/treegraph

## 🛠️ Skills & Technologies

<picture><source media="(prefers-color-scheme: dark)" srcset="https://www.shieldcn.dev/badge/-Go-00ADD8.svg?logo=go&amp;variant=branded&amp;size=sm&amp;mode=dark"><img alt="Go" src="https://www.shieldcn.dev/badge/-Go-00ADD8.svg?logo=go&amp;variant=branded&amp;size=sm&amp;mode=light"></picture> <picture><source media="(prefers-color-scheme: dark)" srcset="https://www.shieldcn.dev/badge/-C-A8B9CC.svg?logo=c&amp;variant=branded&amp;size=sm&amp;mode=dark"><img alt="C" src="https://www.shieldcn.dev/badge/-C-A8B9CC.svg?logo=c&amp;variant=branded&amp;size=sm&amp;mode=light"></picture> <picture><source media="(prefers-color-scheme: dark)" srcset="https://www.shieldcn.dev/badge/-JavaScript-F7DF1E.svg?logo=javascript&amp;variant=branded&amp;size=sm&amp;mode=dark"><img alt="JavaScript" src="https://www.shieldcn.dev/badge/-JavaScript-F7DF1E.svg?logo=javascript&amp;variant=branded&amp;size=sm&amp;mode=light"></picture> <picture><source media="(prefers-color-scheme: dark)" srcset="https://www.shieldcn.dev/badge/-Python-3776AB.svg?logo=python&amp;variant=branded&amp;size=sm&amp;mode=dark"><img alt="Python" src="https://www.shieldcn.dev/badge/-Python-3776AB.svg?logo=python&amp;variant=branded&amp;size=sm&amp;mode=light"></picture> <picture><source media="(prefers-color-scheme: dark)" srcset="https://www.shieldcn.dev/badge/-Rust-000000.svg?logo=rust&amp;variant=branded&amp;size=sm&amp;mode=dark"><img alt="Rust" src="https://www.shieldcn.dev/badge/-Rust-000000.svg?logo=rust&amp;variant=branded&amp;size=sm&amp;mode=light"></picture> <picture><source media="(prefers-color-scheme: dark)" srcset="https://www.shieldcn.dev/badge/-Bash-4EAA25.svg?logo=gnubash&amp;variant=branded&amp;size=sm&amp;mode=dark"><img alt="Bash" src="https://www.shieldcn.dev/badge/-Bash-4EAA25.svg?logo=gnubash&amp;variant=branded&amp;size=sm&amp;mode=light"></picture>

## ☕ Sponsor me 
https://www.paypal.com/paypalme/imartur <br> https://ko-fi.com/panonim

| [<img src="https://raw.githubusercontent.com/Panonim/Panonim/main/Website.svg" alt="Website" width="66"> ](https://bluee.dev/) | [<img src="https://raw.githubusercontent.com/Panonim/Panonim/main/Mail.svg" alt="Mail" width="66">](mailto:office@bluee.dev) |
| :---: | :---:|
<div align="center"><img src="signature-white.svg" height=150px width=250px alt="ArturFlis Signature"/></div>
<br><br>
<a>Tracked since: 12 Jun 2024 11:46</a><a href="https://u8views.com/github/Panonim"><img src="https://u8views.com/api/v1/github/profiles/58903942/views/day-week-month-total-count.svg" align="right"></a>
